...
def train_teacher():
    print("Loading all datasets for Teacher Run (V13)...")
    ...
    # 2. Load Pseudo Labels (Source: V12 Grandmaster - The "Teacher")
    pseudo_path = os.path.join(base_dir, 'submissions', 'submission_v12_grandmaster.csv')
    if not os.path.exists(pseudo_path):
        print("V12 Grandmaster submission not found. Please run blend_grandmaster.py first.")
        return
    pseudo_df = pd.read_csv(pseudo_path)
    
    print(f"Original: {len(orig_df)}, Pseudo (Teacher V12): {len(pseudo_df)}")
    ...
    print("Preprocessing (using V7 Conservative Pipeline)...")
    ...
    # 6. Training Loop
    # Model Params (V7 Tuned) - Keeping same params to isolate the effect of labels
    model_params = {
        'n_estimators': 2000,
        'learning_rate': 0.05,
        'max_depth': 6,
    ...
    print("Starting Teacher Training (V13)...")
    ...
    print(f"\nV13 Teacher (Orig+PseudoV12) CV RMSE: {overall_rmse:.4f}")
    
    # Save OOF
    oof_df = train_df[['id']].copy()
    oof_df['exam_score'] = y_train_only
    oof_df['pred'] = oof_preds
    oof_path = os.path.join(base_dir, 'submissions', 'oof_v13_teacher.csv')
    oof_df.to_csv(oof_path, index=False)
    print(f"OOF predictions saved to '{oof_path}'")
    
    submission['exam_score'] = test_preds
    sub_path = os.path.join(base_dir, 'submissions', 'submission_v13_teacher.csv')
